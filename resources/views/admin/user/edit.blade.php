    					...
    					<form method="post" action="{{ action('UserController@update', $data->id) }}" enctype='multipart/form-data'>
    						<div class="modal-body">
    							...
    							<div class="form-group row">
    								...
    								<div class="col-sm-8">
    									<select class="form-control select2" name="lamaran_id" id="pegawai">
    										...
    										<option value="{{ $value->id }}" {{ $value->id == optional($data->getProfile)->id ? 'selected' : '' }} >{{ $value->nama }}</option>
    										@endforeach
    									</select>
    								</div>
    							</div>
    							...
    							<div class="form-group row">
    								...
    								<div class="col-sm-8">
    									<input required type="text" class="form-control" id="nip" value="{{ optional($data->getProfile)->nip }}" name="nip" readonly>
    								</div>
    							</div>    							
    							<div class="form-group row">
    								<label for="nama" class="col-sm-2 col-form-label">Nama</label>
    								<div class="col-sm-8">
    									<input required type="text" class="form-control" id="nama" value="{{ optional($data->getProfile)->nama  }}" name="nama" readonly>
    								</div>
    							</div>	
    							<div class="form-group row">
    								<label for="jabatan" class="col-sm-2 col-form-label">Jabatan</label>
    								<div class="col-sm-8">
    									<input required type="text" class="form-control" id="jabatan" value="{{ optional(optional($data->getProfile)->getJabatan)->jabatan  }}" name="jabatan" readonly>
    								</div>
    							</div>
    							<div class="form-group row">
    							...
    							<div class="form-group row">
    								...
    								<div class="col-sm-8">
    									<select class="form-control" id="role" name="role">
    										...
    										<option value="{{ $val }}" {{ $data->getRoleNames()->first() == $val ? 'selected' : ''  }}>{{ $val }}</option>
    										@endforeach
    									</select>
    								</div>
    							</div>
